...
for i=1:N
    index = fitnessDistanceBalance(Xwinners, Fitness);
    Fitness(1,i) = testFunction(Xwinners(index,:)', fhd, fNumber);
    ...
    for  i=1:N
        ...
        if alpha<0.5
            ...
            if Memoryless(i,:)==Xwinners(i,:)
                Mu=(X_guide+Memoryless(index,:))/2.0;    %  Eq. (19) is the mean or expected value of an exponentially distributed random vaiable X
                ...
                V(index,:)=a.*(Memoryless(index,:)-variance)+b.*X_guide;   %Eq. (14) first branch
            else
                ...
                V(i,:)=b.*(Memoryless(i,:)-variance)+log (phi).*Xwinners(index,:);% Eq. (14) second branch
            end
        else
            %----------------------------Exploration----------------------------------------
            M=mean(Xwinners);
